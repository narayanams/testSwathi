...
namespace TRAILSWEB.Validators
{
    public static class CommonValidationUtilities
    {
        ...
        public static bool ValidatePhoneField(string fullTenDigigtPhoneNumber)
        {
        ...
        public static bool ValidateCreditCardPaymentInformation(string cardType, string cardNumber, string nameOnCard, string expirationMonth, string expirationYear, decimal amount, bool useCardOnFile, out StringBuilder st)
        {
            ...
            if (!useCardOnFile)
            {
                ...
                {
                    st.Append("Please select a valid credit card type.");
                    return false;
                }
                ...
                {
                    st.Append("Please enter a valid credit card number.");
                    return false;
                }
                ...
                {
                    st.Append("Please enter valid name on Credit Card.");
                    return false;
                }
            ...
            {
                st.Append("Expiration Date should not be less than today's date.");
                return false;
            }
            ...
            {
                st.Append("Please enter an Expiration Date that is for current month and year or greater.");
                return false;
            }
            ...
            if (!expMonth.IsValidExpirationMonth(expYear))
            {
                ...
                st.Append("Please verify the Credit Card Expiration.");
                return false;
            }

            //As par new requirement, we should allow user to make '0' $ payment
            if (decimal.Parse(amount.ToString()) < 0)
            {
                st.Append("Please enter a valid payment amount.");
                return false;
            }

            decimal minmaxAmount = 0M;

            if (!CommonValidationUtilities.IsValidMaximumPaymentAmount(amount.ToString(), out minmaxAmount))
            {
                st.Append(string.Format("Invalid amount {0}. Amount must be equal to or less than {1}.",
                                            amount.ToString("C"), minmaxAmount.ToString("C")));
                return false;
            }
        ...
        public static bool ValidatePaymentInformation(PaymentInformation payment, out StringBuilder st)
        {
            Nullable<DateTime> expirationDate;
            st = new StringBuilder();
            bool isValid = true;
            
            if (payment.IsCCPayment)
            {
                #region CardPayment
                if (!payment.UseCardOnFile)
                {
                    if (payment.CreditCardType == Constants.Common.WebSelectOne)
                    {
                        st.Append("Please select a valid credit card type.");
                        isValid = false;
                    }
                    ...
                    if (string.IsNullOrEmpty(payment.CreditCardNumber)
                        || !CommonValidationUtilities.IsValidCreditCardNumber(payment.CreditCardNumber)
                        || !CommonValidationUtilities.IsValidCrediCardType(payment.CreditCardNumber, payment.CreditCardType))
                    {
                        st.Append("Please enter a valid credit card number.");
                        isValid = false;
                    }
                    ...
                    if (string.IsNullOrEmpty(payment.NameOnCard) || (!CommonValidationUtilities.IsValidName(payment.NameOnCard)))
                    {
                        st.Append("Please enter valid name on Credit Card.");
                        isValid = false;
                    }
                ...
                if (payment.ExpirationMonth.ToString() == Constants.Common.SelectMonth || payment.ExpirationYear.ToString() == Constants.Common.SelectYear)
                {
                    st.Append("Expiration Date should not be less than today's date.");
                    isValid = false;
                }
                expirationDate = new DateTime(int.Parse(payment.ExpirationYear.ToString()), int.Parse(payment.ExpirationMonth.ToString()),
                                       DateTime.DaysInMonth(int.Parse(payment.ExpirationYear.ToString()), int.Parse(payment.ExpirationMonth.ToString())));
                if (expirationDate < DateTime.Now.Date)
                {
                    st.Append("Please enter valid Credit Card Expiration.");
                    isValid = false;
                }

                //Validate Expiration Month/Years
                int expMonth = payment.ExpirationMonth.ToInt();
                int expYear = payment.ExpirationYear.ToInt();
            ...
            {
                #region ACHPayment
                if (string.IsNullOrEmpty(payment.AccountHolderFirstName))
                {
                    st.Append("First Name cannot be empty<BR>");
                    isValid = false;
                }
                if (CommonValidationUtilities.IsValidName(payment.AccountHolderFirstName) == false)
                {
                    st.Append("First Name is invalid.<BR>");
                    isValid = false;
                }
                if (string.IsNullOrEmpty(payment.AccountHolderLastName))
                {
                    st.Append("Last Name cannot be empty<BR>");
                    isValid = false;
                }
                if (CommonValidationUtilities.IsValidName(payment.AccountHolderLastName) == false)
                {
                    st.Append("Last Name is invalid.<BR>");
                    isValid = false;
                }
                if (string.IsNullOrEmpty(payment.BankAccountNumber))
                {
                    st.Append("Account Number cannot be empty<BR>");
                    isValid = false;
                }
                if (CommonValidationUtilities.IsValidBankAccount(payment.BankAccountNumber) == false)
                {
                    st.Append("Account Number is invalid.<BR>");
                    isValid = false;
                }
                if (string.IsNullOrEmpty(payment.RoutingNumber))
                {
                    st.Append("Routing Number cannot be empty<BR>");
                    isValid = false;
                }
                if (CommonValidationUtilities.IsValidRoutingNumber(payment.RoutingNumber) == false)
                {
                    st.Append("Routing Number is invalid.<BR>");
                    isValid = false;
                }
